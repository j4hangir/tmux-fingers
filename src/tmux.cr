...
class Tmux
  ...
  def capture_pane(pane : Pane, join = true, preserve_escapes = false)
    join_flag = join ? "-J" : ""
    escape_flag = preserve_escapes ? "-e" : ""
    if pane.pane_in_mode && !pane.scroll_position.nil?
      ...
      exec("capture-pane #{join_flag} #{escape_flag} -p -t '#{pane.pane_id}' -S #{start_line} -E #{end_line}").chomp
    else
      exec("capture-pane #{join_flag} #{escape_flag} -p -t '#{pane.pane_id}'").chomp
    end
  end
